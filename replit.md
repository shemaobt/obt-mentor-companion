...
- **AI Integration (LangChain Multi-Agent System with Google Gemini 2.5)**:
    ...
    - **Evidence-Based Competency Persistence**: Implemented a `statusSource` enum ('auto', 'manual', 'evidence', 'conversation') to ensure all user-influenced competency updates persist across recalculations.
    ...
    - **Competency Downgrade Protection** (Nov 2025): Enhanced `recalculateCompetencies()` to prevent competency levels from decreasing when adding new qualifications/activities. The system now: (1) preserves manual, evidence-based, and conversation-based competencies without overwriting them, (2) for auto-calculated competencies, only updates if the new score is higher or equal to the existing score, (3) never downgrades competency levels, ensuring monotonic progression or maintenance of achievement levels.
    - **Conversation-Based Update Protection** (Nov 2025): Fixed critical bug where AI-suggested competency updates via `suggest_competency_update` tool were being overwritten by `recalculateCompetencies()`. The system now treats 'conversation' statusSource as protected, preserving AI mentor's evaluations alongside manual and evidence-based updates.
