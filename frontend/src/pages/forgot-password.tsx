          ...
          <form onSubmit={form.handleSubmit((data) => mutation.mutate(data))} className="space-y-5">
            <FormField control={form.control} name="email" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Input type="email" placeholder="you@example.com" autoFocus className="h-12 rounded-xl text-base" {...field} /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
            <Button type="submit" className="w-full h-12 rounded-xl text-base font-semibold active:scale-[0.98] transition-all duration-200" disabled={mutation.isPending}>
              {mutation.isPending ? "Sending..." : (<><Mail className="mr-2 h-4 w-4" /> Send reset link</>)}
            </Button>
          </form>
        </Form>
      )}
