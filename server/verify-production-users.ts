...
async function verifyProductionUsers() {
  ...
  if (!productionUrl) {
    ...
    console.log("Usage: PRODUCTION_DATABASE_URL=your_prod_url tsx server/verify-production-users.ts");
    process.exit(1);
  }
  ...
  try {
    ...
    // Expected count from CSV backup
    ...
    // Get sample of users to verify data integrity
