...
interface ReportData {
  ...
  periodStart: Date;
  periodEnd: Date;
  aiGeneratedNarrative?: string; // Optional AI-generated narrative from LangChain Report Agent
}

export async function generateQuarterlyReport(data: ReportData): Promise<{ filePath: string; document: Document }> {
  const { facilitator, competencies, qualifications, activities, recentMessages, periodStart, periodEnd, aiGeneratedNarrative } = data;
  ...
  // Use AI-generated narrative if provided (LangChain Report Agent)
  if (aiGeneratedNarrative) {
    // Split AI narrative into paragraphs and format them
    const paragraphs = aiGeneratedNarrative.split('\n\n').filter(p => p.trim());
    ...
    console.log('[Report Generator] Using AI-generated narrative from LangChain Report Agent');
  } else {
    // Fallback to template-based narrative (legacy system)
    console.log('[Report Generator] Using template-based narrative (legacy)');
